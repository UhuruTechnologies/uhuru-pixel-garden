...
export default function PixelGridClient() {
  ...
  useEffect(() => {
    ...
    const grid = new PixelGrid(containerRef.current);
    setPixelGrid(grid);
    ...
    const mockPixels = [
      ...
      {
        x: 45, y: 46, color: '#2196F3', height: 4,
        message: 'Watoto Kwanza',
        owner: 'John Smith', timestamp: Date.now() - 86400000 * 2
      },
      {
        x: 46, y: 46, color: '#9C27B0', height: 2,
        message: 'Building the future together',
        owner: 'Jane Smith', timestamp: Date.now() - 86400000
      }
    ];
    
    grid.loadPixelData(mockPixels);
    
    // Handle pixel click to show info
    grid.onPixelClick((x, y, pixelData) => {
      if (pixelData) {
        // Show pixel info (in a real implementation, this would show your popup)
        console.log('Pixel clicked:', pixelData);
      } else {
        // Show editor (in a real implementation, this would show your editor)
        console.log('Empty pixel clicked:', x, y);
      }
    });
...
// PixelGrid class implementation
class PixelGrid {
